...
module.exports = {
  ...
  addToCart: function (sessionID, partNumber, quantity) {
    ...
    db.all(
      'INSERT INTO cartitems (number, quantity_selected, session_id) VALUES (?, ?, ?)',
      [parsedNum, parsedQuan, sessionID],
      (err, rows) => {
        if (err) return console.error(err.message);
  ...
  removeFromCart: function (sessionID, partNumber) {
    let parsedNum = parseInt(partNumber);
    db.all(
      'DELETE FROM cartitems WHERE number = ? AND session_id = ?',
      [parsedNum, sessionID],
      (err, rows) => {
  ...
  getCart: function (sessionID) {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        db.all(
          'SELECT * FROM cartitems INNER JOIN parts ON parts.number = cartitems.number WHERE cartitems.session_id = ?',
          [sessionID],
          (err, rows) => {
            if (err) reject(err);
            resolve(rows);
